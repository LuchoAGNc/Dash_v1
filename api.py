...
async def predict_proba(inputs: List[InputModelo]):
    ...
    for Input in inputs:
        ...
        model = APIModelBackEnd(
            ...
            Input.F_Consumo_verduras, Input.N_comidas, Input.Meriendas, Input.Fumador,
            Input.F_actvidad_fisica, Input.Consumo_calorias, Input.C_alcohol, Input.Medio_transporte
        )
